...
prodchdebye=prod(ch)*Squared_Debye_Huckel;
prodchzmagic=prod(ch)*zmagic;
dime12=(dime(1)-2)*(dime(2)-2);
dime11=(dime(1)-2);
%array to column converter
for i=2:dime(1)-1
    for j=2:dime(2)-1
        for k=2:dime(3)-1
             pe=(k-2)*dime12+(j-2)*dime11+i-1;
             AAxB(pe)=AxB(i,j,k);
             AAyB(pe)=AyB(i,j,k);
             AAzB(pe)=AzB(i,j,k);
             AAMain(pe)=(dielx(i,j,k)+dielx(i-1,j,k))*ch(1)+(diely(i,j,k)+diely(i,j-1,k))*ch(2)...
          +(dielz(i,j,k)+dielz(i,j,k-1))*ch(3)+ kappa(i,j,k)*prodchdebye;
              bb(pe)=charge(i,j,k)*prodchzmagic+fc(i,j,k);
        end
    end
end
...
pcy=-((dime(1)-2)*(dime(2)-3));
pcz=-((dime(1)-2)*(dime(2)-2)*(dime(3)-3));
if ((bx==1 && by==1 && bz==1) || (bx==3 && by==3 && bz==3))
...
if (bx==2 && by==2 && bz==2)
diagonals=horzcat(AAMain,AAxPeriod,AAxB,AAyPeriod,AAyB,AAzPeriod,AAzB);
% Diagonal displacement vector corresponding to the pevious array
d=[dcdiag pcx dcx pcy dcy pcz dcz];
end
...
A=tril(AA,0)+triu(B,1);
 ...
 clear fc AxB AyB AzB AAMain AAxB AAyB AAzB AA B d diagonals
 clear AAxPeriod AAyPeriod AAzPeriod
